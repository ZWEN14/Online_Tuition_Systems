...
public class SubmissionUploadService
{
    ...
    {
        if (files.Count > MaxFiles) throw new InvalidDataException("Choose at most 5 files.");
        var result = new List<SubmissionAttachment>();
        foreach (var file in files)
        {
            if (file.Length is <= 0 or > MaxBytes) throw new InvalidDataException("Each file must be nonempty and no larger than 5 MB.");
            ...
            {
                if (buffer.Length + count > MaxBytes) throw new InvalidDataException("Each file must be no larger than 5 MB.");
            ...
            if (contentType is null) throw new InvalidDataException(photosOnly
