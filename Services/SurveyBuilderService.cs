...
public class SurveyBuilderService(ApplicationDbContext db)
{
    ...
    {
        Id = survey.Id, Title = survey.Title, Description = survey.Description, CourseId = survey.CourseId,
        ExpiresAt = survey.ExpiresAt?.ToLocalTime(), IsActive = survey.IsActive,
        Sections = survey.Sections.OrderBy(x => x.DisplayOrder).Select(s => new SurveySectionCreateViewModel
        {
            Id = s.Id, ClientKey = $"s{s.Id}", Title = s.Title, Description = s.Description,
            ...
            {
                Id = q.Id, ClientKey = $"q{q.Id}", Text = q.Text, Type = q.Type, IsRequired = q.IsRequired,
                Options = q.Options.OrderBy(x => x.DisplayOrder).Select(o => new SurveyBuilderOptionViewModel
                {
                    Id = o.Id, Text = o.Text, DestinationKey = RouteKey(o.BranchRule?.Action ?? SurveyBranchAction.Continue, o.BranchRule?.DestinationSectionId)
                }).ToList()
            }).ToList()
        }).ToList()
    };
    ...
    public async Task ValidateAsync(SurveyCreateViewModel model, Survey? existing, ModelStateDictionary errors)
    {
        ...
        model.Sections ??= [];
        if (model.Sections.Count is < 1 or > 50) Error("Use between 1 and 50 sections.");
        if (model.CourseId.HasValue && !await db.Courses.AnyAsync(x => x.CourseId == model.CourseId)) Error("Select a valid course.");
        var keys = model.Sections.Select(x => x.ClientKey).ToList();
        if (keys.Distinct().Count() != keys.Count || keys.Any(x => string.IsNullOrWhiteSpace(x) || x == "submit")) Error("Section identifiers must be unique.");
        ...
        {
            if (!string.IsNullOrEmpty(key) && key != "submit" && !keys.Skip(sectionIndex + 1).Contains(key)) Error("A route must point to a later section in this survey, or submit the form.");
        }
        for (var i = 0; i < model.Sections.Count; i++)
        {
            var section = model.Sections[i];
            if (section.Id != 0 && (!oldSections.ContainsKey(section.Id) || !usedSections.Add(section.Id))) Error("Invalid or repeated section.");
            ValidateRoute(section.DestinationKey, i);
            section.Questions ??= [];
            if (section.Questions.Count is < 1 or > 100) Error("Each section needs between 1 and 100 questions.");
            var routers = 0;
            foreach (var q in section.Questions)
            {
                oldQuestions.TryGetValue(q.Id, out var old);
                if (q.Id != 0 && (old is null || !usedQuestions.Add(q.Id))) Error("Invalid or repeated question.");
                if (!Enum.IsDefined(q.Type)) Error("Select a valid question type.");
                q.Options ??= [];
                var choice = q.Type is SurveyQuestionType.MultipleChoice or SurveyQuestionType.Dropdown or SurveyQuestionType.Checkbox;
                if (choice)
                {
                    if (q.Options.Count is < 2 or > 50 || q.Options.Select(x => x.Text?.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).Count() != q.Options.Count) Error("Choice questions need 2 to 50 distinct options.");
                    var optionIds = new HashSet<int>();
                    foreach (var o in q.Options)
                    {
                        if (o.Id != 0 && (old is null || !old.Options.Any(x => x.Id == o.Id) || !optionIds.Add(o.Id))) Error("Invalid or repeated answer option.");
                        if (q.Type == SurveyQuestionType.Checkbox && !string.IsNullOrEmpty(o.DestinationKey)) Error("Checkbox questions do not support section routing.");
                        ValidateRoute(o.DestinationKey, i);
                    }
                    if (q.Options.Any(x => !string.IsNullOrEmpty(x.DestinationKey))) routers++;
                }
                else if (q.Options.Count > 0) Error("Only choice questions can have options.");
            ...
            if (routers > 1) Error("Use only one question with option routing in each section.");
        }
        if (answered.Any(x => !usedQuestions.Contains(x))) Error("Questions with submitted answers cannot be removed. You can add new questions or deactivate the survey.");
    }
    ...
    public async Task SaveAsync(SurveyCreateViewModel model, int creatorId)
    {
        await db.Database.CreateExecutionStrategy().ExecuteAsync(async () =>
        {
            ...
            if (model.Id == 0) db.Surveys.Add(survey);
            survey.Title = model.Title.Trim(); survey.Description = model.Description?.Trim(); survey.CourseId = model.CourseId;
            survey.IsActive = model.IsActive; survey.ExpiresAt = model.ExpiresAt?.ToUniversalTime();
            var oldSections = survey.Sections.ToDictionary(x => x.Id);
            var oldQuestions = survey.Questions.ToDictionary(x => x.Id);
            foreach (var section in oldSections.Values) { section.DisplayOrder = -section.Id; section.NextSectionId = null; }
            ...
            for (var i = 0; i < model.Sections.Count; i++)
            {
                ...
                if (input.Id == 0) db.SurveySections.Add(section);
                section.Title = input.Title.Trim(); section.Description = input.Description?.Trim(); section.DisplayOrder = i + 1;
                ...
                for (var j = 0; j < input.Questions.Count; j++)
                {
                    ...
                    if (item.Id == 0) db.Questions.Add(question); else keptQuestions.Add(item.Id);
                    question.Section = section; question.Text = item.Text.Trim(); question.Type = item.Type; question.IsRequired = item.IsRequired; question.DisplayOrder = j + 1;
                    ...
                    for (var k = 0; k < item.Options.Count; k++)
                    {
                        ...
                        if (o.Id == 0) db.QuestionOptions.Add(option);
                        option.Text = o.Text.Trim(); option.DisplayOrder = k + 1; option.BranchRule = null;
