...
public class SurveyController(ApplicationDbContext db, ICurrentUserService currentUser, SubmissionUploadService uploads) : Controller
{
    public async Task<IActionResult> Index()
    {
        ...
        var courseIds = await db.Enrollments.Where(x => x.StudentId == currentUser.UserId).Select(x => x.CourseId).ToListAsync();
    ...
    private Task<bool> CanAccess(Survey survey) => survey.CourseId is null ? Task.FromResult(true) : db.Enrollments.AnyAsync(x => x.StudentId == currentUser.UserId && x.CourseId == survey.CourseId);
