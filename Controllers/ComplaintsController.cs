...
public class ComplaintsController(ApplicationDbContext db, ICurrentUserService currentUser, SubmissionUploadService uploads) : Controller
{
    public async Task<IActionResult> Index(string? status, string? search)
    {
        ...
        if (currentUser.IsInRole("Student")) query = query.Where(x => x.UserId == currentUser.UserId);
        else if (currentUser.IsInRole("Tutor")) query = query.Where(x => x.UserId == currentUser.UserId || x.AssignedTutorId == currentUser.UserId);
        else if (!currentUser.IsInRole("Admin")) return Forbid();
        if (Enum.TryParse<ComplaintStatus>(status, out var parsed)) query = query.Where(x => x.Status == parsed);
        if (!string.IsNullOrWhiteSpace(search)) { search = search.Trim(); query = query.Where(x => x.Title.Contains(search) || x.Description.Contains(search)); }
    ...
    public async Task<IActionResult> Create() { await LoadCategories(); return View(new ComplaintCreateViewModel()); }
    ...
    {
        if (!await db.ComplaintCategories.AnyAsync(x => x.Id == model.CategoryId && x.IsActive)) ModelState.AddModelError(nameof(model.CategoryId), "Select a valid category.");
        var photos = new List<SubmissionAttachment>();
        if (ModelState.IsValid)
        {
            try { photos = await uploads.ReadAsync(model.Photos ?? [], true, HttpContext.RequestAborted); }
    ...
    public async Task<IActionResult> Details(int id)
    {
        ...
        if (complaint is null) return NotFound(); if (!CanView(complaint)) return Forbid(); return View(complaint);
    }

    [HttpGet, Authorize(Roles = "Admin")]
    public async Task<IActionResult> Manage(int id)
    {
        var c = await db.Complaints.FindAsync(id); if (c is null) return NotFound(); await LoadCategories(c.CategoryId, true); await LoadTutors(c.AssignedTutorId);
    ...
    {
        if (!await db.ComplaintCategories.AnyAsync(x => x.Id == model.CategoryId)) ModelState.AddModelError(nameof(model.CategoryId), "Select a valid category.");
    ...
    {
        var c = await db.Complaints.FirstOrDefaultAsync(x => x.Id == id && x.AssignedTutorId == currentUser.UserId); if (c is null) return Forbid();
        if (status is not (ComplaintStatus.InProgress or ComplaintStatus.Resolved)) { TempData["Error"] = "Tutors may only use In Progress or Resolved."; return RedirectToAction(nameof(Details), new { id }); }
        if (note?.Length > 1000) { TempData["Error"] = "The update note cannot exceed 1000 characters."; return RedirectToAction(nameof(Details), new { id }); }
        if (c.Status != status) { c.Status = status; c.UpdatedAt = DateTime.UtcNow; if (!string.IsNullOrWhiteSpace(note)) c.ResolutionNotes = note.Trim(); AddStatusChange(c, status, note); await db.SaveChangesAsync(); }
    ...
    {
        var c = await db.Complaints.FindAsync(id); if (c is null) return NotFound(); if (c.UserId != currentUser.UserId || c.Status != ComplaintStatus.Pending) return Forbid();
        db.Remove(c); await db.SaveChangesAsync(); TempData["Success"] = "Complaint deleted."; return RedirectToAction(nameof(Index));
    }
